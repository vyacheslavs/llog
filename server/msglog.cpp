...
llog::GenericMessagePtr llog::GenericMessage::create(severity sev, const std::string &msg) {
...
std::string llog::GenericMessage::formatted() const {
    ...
    ss << "[" << std::put_time(std::localtime(&in_time_t), "%Y-%m-%d %H:%M:%S") << "." << std::setfill('0') << std::setw(3) << ms.count() << "] " << m_msg;

    return ss.str();
}

llog::MsgLogPtr llog::MsgLog::create(HandlerChainLinkPtr h) {
...
bool llog::MsgLog::log(severity severity, const std::string &message) {
    auto msg = GenericMessage::create(severity, message);
